...
CREATE TABLE jxc_process (
    ...
    default_price1 DECIMAL(10,2) DEFAULT 0 COMMENT '默认单价1',
    default_price2 DECIMAL(10,2) DEFAULT 0 COMMENT '默认单价2',
    default_price3 DECIMAL(10,2) DEFAULT 0 COMMENT '默认单价3',
...
CREATE TABLE jxc_product_process_config (
    ...
    process_id BIGINT NOT NULL COMMENT '工序ID',
    price1 DECIMAL(10,2) DEFAULT 0 COMMENT '单价1',
    price2 DECIMAL(10,2) DEFAULT 0 COMMENT '单价2',
    price3 DECIMAL(10,2) DEFAULT 0 COMMENT '单价3',
...
CREATE TABLE jxc_process_record (
    ...
    base_price DECIMAL(10,2) NOT NULL COMMENT 'SPU基准单价(快照)',
    premium_ratio DECIMAL(5,2) DEFAULT 0 COMMENT 'SKU溢价比例(%)',
    unit_price DECIMAL(10,2) NOT NULL COMMENT '最终单价(快照)=base_price*(1+premium_ratio)',
    share_ratio DECIMAL(5,2) DEFAULT 100 COMMENT '分配比例(%)',
    amount DECIMAL(12,2) NOT NULL COMMENT '计件金额=quantity*unit_price*share_ratio/100',
...
CREATE TABLE jxc_process_record_share (
    ...
    share_amount DECIMAL(12,2) NOT NULL COMMENT '分配金额',
...
CREATE TABLE jxc_sku_price_premium (
    ...
    fixed_price DECIMAL(10,2) COMMENT '固定价格(优先于溢价比例)',
...
CREATE TABLE jxc_rework_piece_work (
    ...
    original_amount DECIMAL(12,2) COMMENT '原计件金额',
    deduction_ratio DECIMAL(5,2) COMMENT '扣款比例(%)',
    deduction_amount DECIMAL(12,2) DEFAULT 0 COMMENT '扣款金额',
    ...
    rework_amount DECIMAL(12,2) COMMENT '返工计件金额',
...
CREATE TABLE jxc_production_cost (
    ...
    overhead_cost DECIMAL(12,2) DEFAULT 0 COMMENT '制造费用',
...
CREATE TABLE jxc_cost_material_detail (
    ...
    amount DECIMAL(12,2),
    wastage_amount DECIMAL(12,2) COMMENT '损耗金额',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
) COMMENT '材料成本明细表';
...
CREATE TABLE jxc_cost_labor_detail (
    ...
    piece_work_amount DECIMAL(12,2) COMMENT '计件金额',
    time_work_amount DECIMAL(12,2) COMMENT '计时金额',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
) COMMENT '人工成本明细表';
...
CREATE TABLE jxc_cost_overhead_detail (
    ...
    allocated_amount DECIMAL(12,2) COMMENT '分摊金额',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
) COMMENT '制造费用明细表';
```
...
| quality_check | quality_check_process | 质检流程 |
| rework_order | rework_process | 返工流程 |
| **bundle_flow** | bundle_production_flow | **扎包工序流转(核心)** |
...
| 工作流节点门禁 | WORKFLOW_NODE_NOT_READY | 节点未就绪 | 禁止计件 |

### 6.4 工作流服务接口

```java
public interface ProductionWorkflowService {
    // 启动扎包生产流程 (根据款式+工厂自动选择模板)
    WfInstance startBundleFlow(Bundle bundle);
    
    // 启动扎包生产流程 (指定模板)
    WfInstance startBundleFlow(Bundle bundle, Long templateId);
    
    // 扫码计件推进流程
    boolean advanceByScanRecord(ProcessRecord record);
    
    // 首件确认通过后推进
    boolean advanceByFirstArticlePassed(Long cuttingPlanId);
    
    // 质检结果推进流程
    boolean advanceByQualityCheck(QualityCheck check);
    
    // 返工完成恢复流程
    boolean resumeByReworkCompleted(Long reworkOrderId);
    ...
    // 解析流程模板 (根据款式+工厂)
    FlowTemplate resolveFlowTemplate(Long styleId, Long factoryId);
}
```
...
CREATE TABLE jxc_unified_inventory (
    ...
    warehouse_id BIGINT NOT NULL COMMENT '仓库ID/车间ID',
    item_type VARCHAR(20) NOT NULL COMMENT '物料类型(fabric/accessory/wip/product)',
    ...
    reserved_qty DECIMAL(12,2) DEFAULT 0 COMMENT '占用数量',
...
CREATE TABLE jxc_inventory_transaction (
    ...
    quantity DECIMAL(12,2) NOT NULL COMMENT '变动数量(正数入库,负数出库)',
    before_qty DECIMAL(12,2) COMMENT '变动前数量',
    after_qty DECIMAL(12,2) COMMENT '变动后数量',
    
    -- 关联业务
    biz_type VARCHAR(30) COMMENT '业务类型(fabric_inbound/cutting/production/quality/sales)',
...
| item_type | 说明 | item_id对应 | 批次/色号/尺码 |
|-----------|------|------------|---------------|
| fabric | 面料库存 | jxc_fabric.fabric_id | 使用batch_no, color |
| accessory | 辅料库存 | jxc_accessory.accessory_id | 使用batch_no |
| wip | 在制品库存 | jxc_bundle.bundle_id | 使用color, size |
| product | 成品库存 | jxc_product_sku.id | 使用color, size |
...
│                                                                          │
│  ① 面料入库                                                             │
│     jxc_fabric_inbound → jxc_unified_inventory (item_type=fabric)      │
│     quantity (+)                                                         │
│                                                                          │
│  ② 面料领用出库                                                         │
│     jxc_fabric_requisition → jxc_unified_inventory (item_type=fabric)  │
│     quantity (-)                                                         │
│                                                                          │
│  ③ 裁床生成扎包 → 在制品入库                                            │
│     jxc_cutting_bundle → jxc_unified_inventory (item_type=wip)         │
│     quantity (+)                                                         │
│                                                                          │
│  ④ 扎包流转 (不改变库存数量，只改变位置/状态)                           │
│     jxc_process_record → jxc_bundle (更新位置)                          │
│                                                                          │
│  ⑤ 质检通过入库 → 成品入库                                              │
│     jxc_quality_check (passed) → jxc_unified_inventory (item_type=product)│
│     在制品 quantity (-), 成品 quantity (+)                               │
│                                                                          │
│  ⑥ 销售发货 → 成品出库                                                  │
│     jxc_sales_order_shipment → jxc_unified_inventory (item_type=product)│
│     quantity (-)                                                         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
CREATE TABLE jxc_salary_sheet (
    ...
    base_salary DECIMAL(12,2) DEFAULT 0 COMMENT '基本工资',
    piece_work_amount DECIMAL(12,2) DEFAULT 0 COMMENT '计件工资',
    overtime_amount DECIMAL(12,2) DEFAULT 0 COMMENT '加班工资',
...
|--------|---------|---------|
| FABRIC | fabric | 面料库存 |
| ACCESSORY | accessory | 辅料库存 |
| WIP | wip | 在制品库存 |
| PRODUCT | product | 成品库存 |
...
CREATE TABLE prod_process_library (
    ...
    standard_price DECIMAL(10,2) NOT NULL COMMENT '标准单价',
...
CREATE TABLE prod_process_price_history (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    process_id BIGINT NOT NULL COMMENT '工序ID',
    old_price DECIMAL(10,2) COMMENT '原单价',
    new_price DECIMAL(10,2) NOT NULL COMMENT '新单价',
...
CREATE TABLE wage_settlement (
    ...
    deduction_amount DECIMAL(12,2) DEFAULT 0 COMMENT '扣款总额',
    bonus_amount DECIMAL(12,2) DEFAULT 0 COMMENT '奖金总额',
...
CREATE TABLE wage_settlement_detail (
    ...
    total_quantity INT DEFAULT 0 COMMENT '总数量',
    piecework_amount DECIMAL(12,2) DEFAULT 0 COMMENT '计件金额',
    deduction_amount DECIMAL(10,2) DEFAULT 0 COMMENT '扣款金额',
    bonus_amount DECIMAL(10,2) DEFAULT 0 COMMENT '奖金',
    final_amount DECIMAL(12,2) DEFAULT 0 COMMENT '应发金额',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
