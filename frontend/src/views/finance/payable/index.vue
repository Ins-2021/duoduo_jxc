  ...
  <div class="payable-page">
    <el-card>
      ...
      <div class="table-operations">
        <el-button type="primary" v-perm="'finance:payable:add'" @click="handleAdd">新增应付</el-button>
      </div>

      <!-- 数据表格 -->
      <el-table :data="tableData" border>
        <el-table-column prop="payableNo" label="应付编号" width="180" />
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="primary" @click="handleWriteOff(row)" :disabled="row.status === 2">核销</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="应付金额">
          <el-input-number v-model="formData.payableAmount" :precision="2" :min="0" />
        </el-form-item>
    ...
    <!-- 核销对话框 -->
    <el-dialog v-model="writeOffDialogVisible" title="应付核销" width="500px">
      <el-form :model="writeOffForm" label-width="100px">
        <el-form-item label="应付金额">
          <el-input :value="formatMoney(currentRow?.payableAmount)" disabled />
        </el-form-item>
...
const handleQuery = async () => {
  try {
    const res = await payableApi.pageList(queryForm)
...
const handleView = async (row: any) => {
  try {
    const res = await payableApi.getById(row.payableId)
    dialogTitle.value = '查看应付'
...
const handleSubmit = async () => {
  try {
    if (formData.payableId) {
      await payableApi.update(formData)
      ElMessage.success('更新成功')
    } else {
      await payableApi.create(formData)
...
const handleWriteOffSubmit = async () => {
  try {
    await payableApi.writeOff(currentRow.value.payableId, writeOffForm.amount)
...
const handleDelete = async (row: any) => {
  try {
    ...
    await payableApi.delete(row.payableId)
