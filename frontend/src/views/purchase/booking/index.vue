        ...
        <el-tab-pane label="按单据" name="by-order">
          ...
          <div class="toolbar" :class="{ 'toolbar-expanded': isOrderSearchExpanded }">
            <div class="left-actions">
              <el-button type="primary" v-perm="'purchase:booking:add'" @click="$router.push('/purchase/booking/add')">新增进货预订单</el-button>
              <el-button v-perm="'purchase:booking:audit'">审核</el-button>
              <el-button v-perm="'purchase:booking:audit'">反审核</el-button>
              <el-button v-perm="'purchase:booking:delete'">删除</el-button>
            </div>
            <div class="right-search">
              ...
              <div class="search-form-row">
                ...
                <el-select v-model="searchOrder.customer" placeholder="供应商" clearable class="search-item">
                  <el-option label="某供应商" value="zsq" />
                </el-select>
              </div>
            ...
            <template v-for="col in orderColumns" :key="col.prop">
              ...
              >
                ...
                <template #default="scope" v-else-if="col.prop === 'status'">
                  <span style="color: #f56c6c; font-weight: bold;">{{ purchaseOrderStatusLabel(scope.row.status) }}</span>
                </template>
                <template #default="scope" v-else-if="col.prop === 'actions'">
                  <div class="action-buttons">
                    <el-button size="small" type="primary">打印</el-button>
                    <el-button size="small" type="danger" v-perm="'purchase:booking:delete'" @click="handleDeleteOrder(scope.row)">删除</el-button>
                  </div>
                </template>
              </el-table-column>
            </template>
...
import { getPurchaseOrderList, deletePurchaseOrder, getPurchaseOrderDetailPage } from '@/api/purchase'
...
const purchaseOrderStatusMap = ref<Record<string, string>>({})
const outStockOptions = ref<any[]>([])

const purchaseOrderStatusLabel = (value: any) => {
  const v = value == null ? '' : String(value)
  return purchaseOrderStatusMap.value[v] ?? v
}

const fetchOrders = async () => {
  try {
    const res = await getPurchaseOrderList(searchOrder)
...
const handleDeleteOrder = (row: any) => {
  ElMessageBox.confirm('确认删除该进货单吗?', '提示', { type: 'warning' }).then(async () => {
    await deletePurchaseOrder(row.orderId)
...
onMounted(async () => {
  try {
    const [statusRes, outRes] = await Promise.all([getDict('purchase_order_status'), getDict('out_stock_status')])
    const map: Record<string, string> = {}
    ;(statusRes.data || []).forEach((i: any) => {
      map[String(i.value)] = i.label
    })
    purchaseOrderStatusMap.value = map
    outStockOptions.value = outRes.data || []
  } catch {
    purchaseOrderStatusMap.value = {}
...
const fetchDetailData = async () => {
  try {
    const res = await getPurchaseOrderDetailPage({
      ...searchDetail,
      orderType: 2 // 进货预订单明细
    })
