    ...
    <el-card class="box-card">
      <div class="top-bar">
        <div class="left-actions">
          <span class="page-title"><el-icon><Tickets /></el-icon> 进货退货单</span>
      ...
      <div class="detail-table-area">
        <el-table :data="formData.details" border style="width: 100%" class="custom-table" show-summary :summary-method="getSummaries">
          ...
          <el-table-column label="辅助属性" width="100" align="center" />
          <el-table-column label="规格型号" width="100" align="center" />
          <el-table-column label="单位" width="80" align="center" />
          <el-table-column label="商品批次" width="100" align="center" />
          
          <el-table-column v-for="(label, idx) in sizeColumnLabels" :key="'col'+(idx+1)" :label="label" width="50" align="center">
            <template #default="scope">
          ...
          <el-table-column label="*数量" width="100" align="center">
            <template #default="scope">
              <el-input v-model="scope.row.qty" readonly class="qty-input" />
            </template>
          </el-table-column>
          ...
          <el-main class="product-main">
            <div class="product-toolbar">
              <div class="toolbar-left">
                <el-button type="success" size="small">添加商品</el-button>
            ...
            <SizeColorMatrixPicker v-if="enableSizeColor" :products="filteredProducts" v-model="matrixQtyMap" />
            <el-table
              v-else
...
import { addProduct, getProductDetail, getProductCategoryTree, getProductSkuPage } from '@/api/product'
...
const saveOrder = async () => {
  ...
  const supplierId = Number(formData.customer)
  if (!supplierId) {
  ...
  const ensureProductIds = async (item: any) => {
    ...
    if (!allowNewProductOnBilling.value) {
      throw new Error('请先通过选择商品录入有效商品')
    }
      ...
      skuList: [
        {
          ...
          retailPrice: Number(item.unitPrice) || 0,
  ...
  try {
    ...
    ElMessage.error(e?.message || '单据保存失败')
    console.error(e)
  }
}
...
onMounted(async () => {
  try {
    ...
    enableSizeColor.value = !!res?.data?.enableSizeColor
  ...
  await Promise.all([fetchSuppliers(), fetchCategories(), fetchProducts()])
})
...
const confirmAddCustomer = async () => {
  ...
  try {
    const res = await addSupplier({
      supplierName: name,
      status: addCustomerForm.status === 'enabled' ? 1 : 0
    })
    const id = Number(res?.data)
    customers.value.unshift({ id, name })
    formData.customer = String(id)
    resetCustomerPage()
...
const categoryTree = ref([{ id: 0, label: '全部分类', children: [] as any[] }])
...
const productColumns = ref<ColumnConfig[]>([
  ...
  { prop: 'wholesalePrice', originalLabel: '销售价格', label: '销售单价', visible: true, width: 110 },
  { prop: 'purchasePrice', originalLabel: '进货价格', label: '进货价格', visible: true, width: 110 },
...
const fetchCategories = async () => {
  try {
    ...
    console.error(e)
    categoryTree.value = [{ id: 0, label: '全部分类', children: [] }]
  }
}

const fetchProducts = async () => {
  try {
    const res = await getProductSkuPage({
      pageNum: productPager.page,
      pageSize: productPager.pageSize,
      keyword: productSearch.keyword || undefined,
      categoryId: selectedCategoryId.value || undefined
    })
    allProducts.value = (res?.data?.list || []).map((p: any) => ({
      id: Number(p.id),
      ...
      location: p.location || '',
      exchangePoints: Number(p.exchangePoints || 0),
      stockUnit: p.stockUnit || '',
      stockValue: String(p.stockValue ?? 0),
      remark: p.remark || '',
      selectedQty: 1
    }))
...
const onCategoryClick = (node: any) => {
  selectedCategoryId.value = Number(node?.id || 0) || null
  productPager.page = 1
  fetchProducts()
}
...
const confirmSelectProducts = () => {
  ...
  if (enableSizeColor.value) {
    ...
    const groups = new Map<string, { name: string; color: string; unitPrice: string; sizeQty: Record<string, number>; spuId: number; skuId: number }>()
    selectedSkus.forEach(({ p, qty }) => {
      ...
      if (!groups.has(key)) {
        groups.set(key, { name: p.name, color, unitPrice, sizeQty: {}, spuId: p.spuId, skuId: p.skuId })
      }
    ...
    Array.from(groups.values()).forEach(g => {
      ...
      targetRow.spuId = g.spuId
      targetRow.skuId = g.skuId
