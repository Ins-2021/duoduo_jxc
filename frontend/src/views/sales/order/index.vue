        ...
        <el-tab-pane label="按单据" name="by-order">
          ...
          <div class="toolbar" :class="{ 'toolbar-expanded': isOrderSearchExpanded }">
            <div class="left-actions">
              <el-button type="primary" v-perm="'sales:order:add'" @click="$router.push('/sales/order/add')">新增销售单</el-button>
              <el-button v-perm="'sales:order:audit'">审核</el-button>
              <el-button v-perm="'sales:order:audit'">反审核</el-button>
              <el-button v-perm="'sales:order:delete'">删除</el-button>
            </div>
          ...
          >
            <template v-for="col in orderColumns" :key="col.prop">
              ...
              >
                <!-- 针对特殊列的自定义插槽 -->
                <template #default="scope" v-if="col.prop === 'docNo'">
                  <el-link type="primary" :underline="false">{{ scope.row.docNo }}</el-link>
                </template>
                ...
                <template #default="scope" v-else-if="col.prop === 'actions'">
                  <div class="action-buttons">
                    ...
                    <el-button size="small" v-perm="'sales:return:add'" @click="handleCreateReturn(scope.row)">退货</el-button>
                    <el-button size="small" type="danger" v-perm="'sales:order:delete'" @click="handleDeleteOrder(scope.row)">删除</el-button>
                  </div>
                </template>
              </el-table-column>
            </template>
...
import { getSalesOrderList, deleteSalesOrder, getSalesOrderDetailPage } from '@/api/sales'
import { getViewConfig, resetViewConfig, saveViewConfig } from '@/api/ui'
import { getDict } from '@/api/dict'
import { ElMessage, ElMessageBox } from 'element-plus'

const activeTab = ref('by-order')
const router = useRouter()
const ORDER_VIEW_KEY = 'sales_order_list_by_order'
const DETAIL_VIEW_KEY = 'sales_order_list_by_detail'
...
const orderColumns = ref<ColumnConfig[]>([
  ...
  { prop: 'products', originalLabel: '销售商品', label: '销售商品', visible: true, minWidth: 200 },
...
const openSettingDialog = async (type: 'order'|'detail') => {
  currentSettingType.value = type
  const viewKey = type === 'order' ? ORDER_VIEW_KEY : DETAIL_VIEW_KEY
  const res = await getViewConfig(viewKey)
  currentSettingColumns.value = JSON.parse(JSON.stringify(res.data.columns || []))
...
const refreshColumns = async () => {
  const [orderRes, detailRes] = await Promise.all([getViewConfig(ORDER_VIEW_KEY), getViewConfig(DETAIL_VIEW_KEY)])
  orderColumns.value = (orderRes.data.columns || []) as ColumnConfig[]
  detailColumns.value = (detailRes.data.columns || []) as ColumnConfig[]
}

const saveSettings = async () => {
  const viewKey = currentSettingType.value === 'order' ? ORDER_VIEW_KEY : DETAIL_VIEW_KEY
  await saveViewConfig(viewKey, currentSettingColumns.value)
...
const handleDeleteOrder = (row: any) => {
  ElMessageBox.confirm('确认删除该销售单吗?', '提示', { type: 'warning' }).then(async () => {
...
const handleCreateReturn = (row: any) => {
  router.push({
    ...
    query: {
      sourceOrderId: String(row.orderId)
    }
  })
}
